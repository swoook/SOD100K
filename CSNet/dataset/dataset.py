...
class ImageDataTrain(data.Dataset):
    ...
    def __getitem__(self, item):
        # sal data loading
        im_name, gt_name = self.sal_list[item % self.sal_num].split()
...
def load_image(path):
    ...
    # read a image file in BGR format.
    im = cv2.imread(path)
    # convert bgr2rgb to suit for pytorch pretrained model.
...
def load_image_test(path):
    if not os.path.exists(path):
        print('File {} not exists'.format(path))
    im = cv2.imread(path)
    # form rgb to bgr to suit for pytorch pretrained model.
    im = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    in_ = np.array(im, dtype=np.float32)
    in_ = in_/255
    in_ -= np.array((0.485, 0.456, 0.406))
